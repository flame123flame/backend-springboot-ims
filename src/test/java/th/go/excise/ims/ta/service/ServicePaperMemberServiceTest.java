...
public class ServicePaperMemberServiceTest {
	
	private static final String OUTPUT_PATH = "/tmp/excise/ims/report";
	
	@Autowired
	private ServicePaperMemberService servicePaperMemberService;
	...
	public void test_inquiry() {
		...
		ServicePaperVo vo = servicePaperMemberService.inquiry(formVo);
	...
	public void test_export() {
		...
		String fileName = "servicePaper03_" + LocalDate.now().format(DateTimeFormatter.BASIC_ISO_DATE) + ".xlsx";
		
		try (FileOutputStream os = new FileOutputStream(OUTPUT_PATH + "/" + fileName)) {
			byte[] bytes = servicePaperMemberService.export(formVo);
			os.write(bytes);

...
public class ServicePaperBalanceGoodsServiceTest {
	...
	@Autowired
	private ServicePaperBalanceGoodsService servicePaperBalanceGoodsService;
	...
	public void test_inquiry() {
		...
		ServicePaperVo vo = servicePaperBalanceGoodsService.inquiry(formVo);
	...
	public void test_export() {
		...
		String fileName = "servicePaper04_" + LocalDate.now().format(DateTimeFormatter.BASIC_ISO_DATE) + ".xlsx";
		
		try (FileOutputStream os = new FileOutputStream(OUTPUT_PATH + "/" + fileName)) {
			byte[] bytes = servicePaperBalanceGoodsService.export(formVo);
			os.write(bytes);

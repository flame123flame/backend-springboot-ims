...
public class Int090303Service {
	...
	private Logger logger = LoggerFactory.getLogger(Int090303Service.class);
	...
	public byte[] exportData(Int0903FormVo formVo) {
		...
		List<IaCheckControlRegis> dataList = list(formVo);
		...
		cell.setCellValue("ทะเบียนคุมเช็ค");
		...
		cellNum = 0;
		int no = 1;
		for (IaCheckControlRegis item : dataList) {
			row = sheet.createRow(rowNum);
			// No.
			cell = row.createCell(cellNum);
			...
			cell = row.createCell(cellNum);
			cell.setCellValue(item.getPaymentDate() != null ? ConvertDateUtils.formatDateToString(item.getPaymentDate(), ConvertDateUtils.DD_MM_YYYY, ConvertDateUtils.LOCAL_TH) : "");
			cell.setCellStyle(cellCenter);
			cellNum++;
